...
import datetime
import sys
sys.path.append("/Users/hzhaotibco.com/codecommit/raven/src")
...
def handler(event, context):
    ...
    #message_s = "Storage Common Dir and Local postgrebackup update"
    #message = ' ' + message_s + ' ' ' ''\n'   
    message = """    
    ...
    for s1 in storage_buckets:
        ...
        for s3 in storage_files:
            ...
            if any(s3 in s for s in temp_list):
                ...
                mid_list.append(s3)
                #message += ' ' + s3 + ' ' + message_2 +  ' ''\n'
            #not_present.append(s3)
    ...
    for l1 in logs_buckets:
        ...
        for l2 in resp['Contents']:
            if l2['LastModified'].replace(tzinfo=None) > comp_date.replace(tzinfo=None):
                #message += ' ' + l2['Key'] + ' ' + l2['LastModified'].strftime('%m/%d/%Y %H:%M:%S') + ' ' ' ''\n'
                key1 = l2['Key']
    ...
    if alert:
        ...
        scroll = Scroll(Severity.Alert)
    else:
        #scroll = raven.Scroll(raven.Severity.Report)
        scroll = Scroll(Severity.Report)
